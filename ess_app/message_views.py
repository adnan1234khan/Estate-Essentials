...
def compose(request):
    ...
    if request.method=="POST":
       ...
       User_message=user_Message(sender_id=sender_id,receiver_id=receiver_id,receiver_status=True,sender_status=True,subject=subject,content=content) #pass all the values 
       User_message.save()
...
def delete_sentitem(request):
     if request.method=='POST':
      ...
      for id in message_id_list:
         message_object=user_Message.objects.get(id=id)
