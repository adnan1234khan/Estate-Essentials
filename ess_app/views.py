...
# for home page
def home(request):
  scheme_list=Scheme.objects.all() #it is used to fetch all object from table
  ...
  return render(request,'ess_app/html/index.html',scheme_dict)#passing the dictionary to html page

def contact(request):
  return render(request,'ess_app/html/contact.html')
...
def contactus(request):
  ...
  if request.method=='POST':
    user_name=request.POST["name"] #request.POST is a dict & control(text/textbox etc) name is the key
    user_email=request.POST["email"]
    user_phone=request.POST["phone"]
    user_query=request.POST["query"]
    query=Query(name=user_name,email=user_email,phone=user_phone,user_query=user_query)#creating object
    query.save()
    print("data saved")
...
def feedback(request):
  ...
  if request.method=='POST':
    ...
    feed=Feedback_Rating(name=user_name,email=user_email,phone=user_phone,feedback_text=user_feedback,ratings=user_ratings)#creating object
    feed.save()
    print("data saved")
...
def test(request):
   return render(request,'ess_app/vastu/vastu_base.html')
...
def citywise_rentedhouse(request):
   ...
   return render(request,'ess_app/html/specific_property.html',context)
...
def ml_predict(request):
      ...
      if request.method=="POST":
            ...
            messages.success(request, f"According to given data the predicted price is â‚¹ {predicted_price}  ")
      return render(request,'ess_app/html/priceprediction.html',context)
